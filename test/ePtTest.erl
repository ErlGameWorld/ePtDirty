...
-record(player2, {
    ...
    mp :: integer(),           %% bit=8
    name :: string(),          %% bit=16
    dirty_flag = 0 :: integer() %% bit=32
}).

%% Record 3: Uncontrolled record (no dirty_flag)
-record(item, {
    id,
    count
}).
...
upd_manual_dirty() ->
    P0 = mk(),
    P0#player{name = <<"x">>, dirty_flag = 7}.

upd_only_dirty() ->
    P0 = mk(),
    P0#player{dirty_flag = 5}.
...
upd_var_single_field_sets_dirty_test() ->
    ...
    ?assertEqual(?BIT_NAME, P#player.dirty_flag).

upd_var_multi_fields_sets_or_mask_test() ->
    P = upd_var_multi(),
    ?assertEqual(<<"a">>, P#player.name),
    ?assertEqual(2, P#player.level),
    ?assertEqual(?BIT_NAME bor ?BIT_LEVEL, P#player.dirty_flag).

upd_chain_updates_accumulate_dirty_test() ->
    P = upd_chain(),
    ?assertEqual((?BIT_NAME bor ?BIT_HP), P#player.dirty_flag).

manual_dirtyflag_skips_injection_test() ->
    P = upd_manual_dirty(),
    ?assertEqual(<<"x">>, P#player.name),
    ?assertEqual(7, P#player.dirty_flag).

only_dirtyflag_update_skips_injection_test() ->
    P = upd_only_dirty(),
    ?assertEqual(5, P#player.dirty_flag).

or_with_existing_dirtyflag_test() ->
    P0 = mk(),
    P1 = P0#player{dirty_flag = 3},
    P2 = upd_hp(P1),
    ?assertEqual(19, P2#player.dirty_flag).
...
nested_expr_update_evaluated_once_test() ->
    ...
    ?assertEqual(1, Cnt),
    ?assertEqual(?BIT_LEVEL, P#player.dirty_flag).
...
list_comprehension_update_test() ->
    P0 = mk(),
    P1 = P0#player{dirty_flag = 1},
    [A, B] = list_comp_inc_hp([P0, P1]),
    ?assertEqual(?BIT_HP, A#player.dirty_flag),
    ?assertEqual(1 bor ?BIT_HP, B#player.dirty_flag).

lists_map_fun_update_test() ->
    P0 = mk(),
    [P1] = map_inc_level([P0]),
    ?assertEqual(2, P1#player.level),
    ?assertEqual(?BIT_LEVEL, P1#player.dirty_flag).
...
identity_update_test() ->
    ...
    P1 = P0#player{},
    ?assertEqual(P0, P1),
    ?assertEqual(0, P1#player.dirty_flag).

manual_dirty_reset_test() ->
    P0 = mk(),
    %% Update name (bit 4), but set dirty_flag to 0 manually
    %% Logic: if dirty_flag is explicitly set, automatic injection is skipped.
    P1 = P0#player{name = <<"b">>, dirty_flag = 0},
...
basic_update_test() ->
    P0 = #player2{id = 101, hp = 100, mp = 100, dirty_flag = 0},
    %% Update hp (bit: 4)
    P1 = P0#player2{hp = 90},
    ?assertEqual(90, P1#player2.hp),
    ?assertEqual(4, P1#player2.dirty_flag),
    %% Update mp (bit: 8) -> 4 bor 8 = 12
    P2 = P1#player2{mp = 50},
    ?assertEqual(12, P2#player2.dirty_flag).

multi_field_update_test() ->
    P0 = #player2{id = 101, hp = 100, mp = 100, dirty_flag = 0},
    %% id(2) + name(16) -> 18
    P1 = P0#player2{id = 102, name = "Hero"},
    ?assertEqual(18, P1#player2.dirty_flag).

creation_safety_test() ->
    P = #player2{hp = 100},
    ?assertEqual(0, P#player2.dirty_flag).

pattern_safety_test() ->
    P = #player2{hp = 0, dirty_flag = 0},
    Result = case P of
                 #player2{hp = 0} -> dead;
                 _ -> alive
             end,
...
manual_override_test() ->
    P0 = #player2{hp = 100, dirty_flag = 255},
    P1 = P0#player2{hp = 50, dirty_flag = 0},
    ?assertEqual(0, P1#player2.dirty_flag).
...
complex_expr_test() ->
    ...
    ?assertEqual(4, P#player2.dirty_flag).
...
advanced_pattern_match_test() ->
    ...
    %% 4. Verify no dirty_flag check was injected (implicit)
    %% If injection happened, it would likely be syntactically invalid or logic would fail
    %% e.g., if it tried to match dirty_flag against an expression.

    %% Explicitly matching a record that HAS dirty flags set, but we ignore them in pattern
    P_Dirty = P0#player{dirty_flag = 999},
    Res2 = case P_Dirty of
               #player{id = 0} -> id_0; %% Should match regardless of dirty_flag
               _ -> fail
           end,
